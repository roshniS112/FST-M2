--Load Data from HDFS
inputDialogue4= LOAD 'hdfs:///user/roshni/input/episodeIV_dialogues.txt' USING PigStorage('\t') AS
(name:chararray, line:chararray);
inputDialogue5= LOAD 'hdfs:///user/roshni/input/episodeV_dialogues.txt' USING PigStorage('\t') AS
(name:chararray, line:chararray);
inputDialogue6= LOAD 'hdfs:///user/roshni/input/episodeVI_dialogues.txt' USING PigStorage('\t') AS
(name:chararray, line:chararray);
...
rmf hdfs:///user/roshni/outputs;

-Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/roshni/outputs' USING PigStorage('\t');